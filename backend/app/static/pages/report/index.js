    ...
    for (const group of cr.issue_groups) {
      for (const item of group.items || []) {
        ...
        out.push({
          ...
          rule_name_zh: item.rule_name_zh || (item.rule && item.rule.name_zh) || "",
...
function renderFindings(root, findings, opts) {
  ...
  slot.innerHTML = findings.map((f, idx) => {
    ...
    const title = f.rule_name_zh || f.violated_rule_id || f.rule_id || "—";
    // 副标题：法规出处 GB 编号 / 条文
    const subRef = f.regulation_ref || "";
    // 法规原文：单独引用块
    const regExcerpt = f.regulation_excerpt || "";
    // 兜底：旧 rule_text（无 excerpt 时显示）
    const ruleQuote = !regExcerpt && f.rule_text ? f.rule_text : "";
    return `
        ...
        <div class="body">
          <div class="row">
            ...
            <div class="v">
              ...
              ${regExcerpt ? `<blockquote>${escapeHtml(regExcerpt)}</blockquote>` : ""}
              ${ruleQuote ? `<blockquote>${escapeHtml(ruleQuote)}</blockquote>` : ""}
            </div>
          </div>
