...
const SCENARIOS = [
  ...
  { id: "supplier_intake",      label: "供应商资料准入（SDS / 资质核验）" },
  { id: "process_introduction", label: "工艺导入风险评估" },
];

const MARKETS = [
  { id: "CN", label: "CN · 中国大陆" },
  { id: "EU", label: "EU · 欧盟（REACH / CLP）" },
  { id: "US", label: "US · 美国（TSCA）" },
  { id: "KR", label: "KR · 韩国（K-REACH）" },
  { id: "JP", label: "JP · 日本（化审法）" },
];
...
export async function renderStep2(outlet, ctx) {
  ...
  outlet.innerHTML = `
    <div class="wzbody">
      <h1>上传资料包</h1>
      <div class="lead">把这次审查涉及的 SDS、配方表、工艺说明等一并丢进来，系统会自动识别类型并预检完整性。</div>
      <section>
...
const SCOPE_GROUPS = [
  {
    key: "material", title: "物料", items: [
      { id: "ingredient_identity",   label: "组分清单与 CAS 一致性核对", sub: "所有声明的 CAS 号在权威数据库可查验", rule: "GB/T 17519 · 第 3 条", real: true, recommended: true },
      { id: "restricted_substance",  label: "EU 受限物质比对（REACH 附录 XVII）", sub: "本场景必选 · 由目标市场触发", rule: "REACH 附录 XVII", real: true, recommended: true },
      { id: "svhc_check",            label: "EU SVHC 候选物清单比对", sub: "对照最近一版 SVHC 名录", rule: "REACH 第 59 条", real: false, recommended: true },
  ...
  },
  {
    key: "process", title: "工艺", items: [
      ...
      { id: "voc_limit",             label: "挥发性有机化合物排放限值", sub: "水性涂料 VOC ≤ 80 g/L", rule: "GB 18582-2020", real: false, recommended: true },
      { id: "energy_footprint",      label: "能效与碳足迹估算（可选）", sub: "非合规必检，仅供参考", rule: "非强制", real: false, recommended: false },
    ],
  },
  {
    key: "storage", title: "储运", items: [
      { id: "storage_transport",     label: "运输分类与 UN 编号匹配", sub: "多式联运危险货物分类", rule: "JT/T 617 · UN 1263", real: true, recommended: true },
      { id: "compatibility_risk",    label: "物料相容性 / 不相容物声明", sub: "SDS 第 7 节 / 第 10 节", rule: "SDS Sec 7,10", real: true, recommended: true },
      { id: "sds_key_sections",      label: "包装标签 GHS 要素完整性", sub: "象形图 / 信号词 / H/P 短语", rule: "GHS 修订版 9", real: true, recommended: true },
      { id: "emergency_plan",        label: "应急响应 / 泄漏处置预案", sub: "SDS 第 6 节", rule: "SDS Sec 6", real: false, recommended: false },
    ],
  },
  {
    key: "regulatory", title: "法规适配", items: [
      { id: "regulatory_screening",  label: "目标市场法规匹配（EU/US/CN 等）", sub: "REACH / TSCA / 国内目录联合筛查", rule: "REACH 第 5,6 条", real: true, recommended: true },
      { id: "supplier_evidence_consistency", label: "供应商声明 / 检测报告一致性", sub: "下游用户告知义务与一致性", rule: "REACH 第 31,38 条", real: true, recommended: true },
      ...
      { id: "label_lang",            label: "标签语言版本适配", sub: "EU 27 国语言要求", rule: "CLP 第 17 条", real: false, recommended: false },
      { id: "tsca_check",            label: "US TSCA 化学品申报", sub: "未勾选 US 时建议跳过", rule: "TSCA Sec 5", real: false, recommended: false },
    ],
  },
];
...
export async function renderStep4(outlet, ctx) {
  ...
  nextBtn.addEventListener("click", () => {
    if (nextBtn.hasAttribute("disabled")) return;
    navigate(`#/cases/${encodeURIComponent(ctx.caseId)}/new?step=5`);
  });
}
...
export async function renderStep5(outlet, ctx) {
  ...
  runBtn.addEventListener("click", () => kickoff());
}
