...
function pickRuleType(meta) {
  const t = (meta && (meta.type || meta.rule_type || meta.category)) || "";
  return String(t || "规则");
}
...
function groupChunksToRules(chunks) {
  ...
  for (const c of chunks || []) {
    ...
    const rid = meta.rule_id || meta.ruleId || meta.source_id || c.rule_id
      || meta.source || c.source || meta.document || c.id || "—";
    const name = meta.name || meta.title || meta.rule_name || meta.source_title
      || c.title || rid;
...
function paintStatus(outlet, st) {
  ...
  const sourceCount = (st && (st.source_count ?? st.sourceCount)) ?? 0;
  const chunkCount = (st && (st.chunk_count ?? st.chunkCount)) ?? 0;
  const vectorCount = (st && (st.vector_count ?? st.vectorCount)) ?? 0;
  const packId = (st && (st.pack_id ?? st.packId)) || "—";
  const provider = (st && (st.embedding_provider ?? st.embeddingProvider)) || "—";
  const model = (st && (st.embedding_model ?? st.embeddingModel)) || "—";
...
function bindSearch(outlet) {
  ...
  const doSearch = async () => {
    ...
    try {
      ...
      const items = Array.isArray(resp) ? resp : (resp && (resp.results || resp.hits || resp.chunks)) || [];
      if (!items.length) {
      ...
      out.innerHTML = items.map((it, i) => {
        ...
        const rid = meta.rule_id || meta.source || it.rule_id || it.id || `#${i + 1}`;
...
function bindUploadModal(outlet) {
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    if (!mf) { msg.textContent = "请选择 Manifest YAML 文件"; msg.className = "kb-modal-msg err"; return; }
